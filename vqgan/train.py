...
def save_checkpoint(path: Path, step: int, epoch: int, batch_offset: int, model, discriminator,
...
def main() -> None:
    ...
    model = VQGAN(config.model, grad_checkpointing=config.train.grad_checkpointing).to(device)
    ...
    generator_params = list(model.encoder.parameters()) + list(model.decoder.parameters())
    if not config.model.use_ema:
        generator_params += list(model.quantizer.parameters())
    ...
    effective_batch_size = config.train.batch_size * config.train.grad_accum_steps
    print(
        f"physical_batch_size={config.train.batch_size} "
    ...
    # changes later (e.g. to fit VRAM after lowering physical batch size)
    ...
    while step < config.train.total_steps:
        ...
        for batch_idx, (images, batch_bucket_ids) in enumerate(loader):
            ...
            if is_accum_boundary:
                ...
                # sync so the measured duration reflects actual GPU-bound compute time for
                # this full accumulation cycle, not just CPU kernel-launch time
                if device.type == "cuda":
                ...
                if step % config.train.checkpoint_every == 0:
                    ...
                    save_checkpoint(
                        ckpt_dir / f"step_{step}.pt", step, epoch, consumed, model, discriminator,
                        opt_g, opt_d, config,
                    )
                    save_checkpoint(
                        ckpt_dir / "latest.pt", step, epoch, consumed, model, discriminator,
                        opt_g, opt_d, config,
                    )

                if val_dataset is not None and step % config.train.val_every == 0:
                    run_validation(model, val_dataset, buckets, device, writer, step, config)
                    model.train()
