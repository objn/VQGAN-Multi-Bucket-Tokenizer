...
# $HOME/vqgan_data), then starts training via train.sh (plain python, no uv) with
# --train-dir/--val-dir pointed at that local dataset. See train_with_coco_mini_uv.sh
# for the uv-managed equivalent.
#
...
./train.sh --train-dir "$TRAIN_DIR" --val-dir "$VAL_DIR" "$@"
